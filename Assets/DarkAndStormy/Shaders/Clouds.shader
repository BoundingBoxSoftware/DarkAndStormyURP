	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 SampleClouds ( float3 uv, float4 tiling1, float4 tiling2, float4 tiling3, float4 tiling4, half3 sunTrans, half densityAdd ){
				...
				float time = _Time.y * _DistSpeed * _Speed * 0.1;
				float dist = _DistAmount;
				float timeFrac1 = frac( time );
				float timeFrac2 = frac( time + 0.5 );
			...
			fixed4 frag (v2f IN) : SV_Target
			{
				...
				float time = _Speed * _Time.y * 0.01;

				float oneOverScale = 1.0 / _Scale;
				float4 tiling1 = float4( _Tiling1.xy, _Tiling1.zw * time );
				float4 tiling2 = float4( _Tiling2.xy, _Tiling2.zw * time );
				float4 tiling3 = float4( _TilingWave.xy, _TilingWave.zw * time );
				float4 tiling4 = float4( _TilingColor.xy, _TilingColor.zw * time );
